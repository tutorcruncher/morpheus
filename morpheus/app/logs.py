...
def setup_logging(settings: Settings):
    ...
    if settings.raven_dsn:
        client = Client(
            transport=QueuedAioHttpTransport, dsn=settings.raven_dsn, release=settings.commit, name=settings.deploy_name
        )
    config = {
        'version': 1,
        ...
        'loggers': {
            'morpheus': {'handlers': ['morpheus.default', 'sentry'], 'level': settings.log_level},
            'arq': {'handlers': ['morpheus.default', 'sentry'], 'level': settings.log_level},
        },
    }
